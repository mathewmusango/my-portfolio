...
### Changed
- **One deploy workflow, two environments** — `deploy-staging-s3.yml` + `deploy-pre-prod-s3.yml` + `deploy-prod-pages.yml` collapse into a single `deploy.yml` with three jobs (`aws-s3-staging`, `aws-s3-prod`, `github-pages-prod`) behind one `detect` job that answers the target once — the two prod jobs read that single answer instead of repeating the condition — now the only place a deploy is defined. The retired `pre-prod` name is gone for good: the AWS plane (S3 + CloudFront, `<project>-prod-site`) has always *been* prod, so it is named prod. **Both prod planes are gated** by the `prod` environment's required reviewer — staging stays automatic — and because gating the AWS plane moves its OIDC `sub` to the environment form, the prod deploy trust gained `environment:prod` first (`scripts/bootstrap_aws.sh`; re-run the prod bootstrap before this lands) (#57). The three jobs are thin — trigger, gate, permissions, one `uses:` — and the shared logic lives in two composite actions (`.github/actions/aws-s3`, `.github/actions/github-pages`), which the staging and prod S3 targets share through an environment token; each job checks the repo out first, because a `uses: ./...` action is resolved from the job workspace. Composite actions rather than library reusables, because a reusable-calling job cannot carry `environment:` and the reviewer gate must stay in this repo.
